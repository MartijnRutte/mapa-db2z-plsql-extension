...
parser grammar DB2zPLSQLParser;

options {tokenVocab=DB2zPLSQLLexer;}

startRule : (adminCmdDsn | adminCmdDb2 | dsnutilCall)* | EOF ;
...
admCmdDsnReturnCode
	: (ADM_CMD_DSN_SIMPLEQUOTE? ADM_CMD_DSN_INTEGER  ADM_CMD_DSN_SIMPLEQUOTE? | ADM_CMD_DSN_EMPTYARG)
	;

bind
	: ADM_CMD_DSN_BIND (bindOptions continuation?)+
	;
...
enable
	: ADM_CMD_DSN_ENABLE arg
	;

encoding
	: ADM_CMD_DSN_ENCODING arg
	;

explain
	: ADM_CMD_DSN_EXPLAIN arg
	;
...
member
	:ADM_CMD_DSN_MEMBER arg
	;

name
	: ADM_CMD_DSN_NAME arg
	;
...
copyoptions
	: ADM_CMD_DSN_OPTIONS arg
	;

owner
	: ADM_CMD_DSN_OWNER arg
	;

dsnpackage
	: ADM_CMD_DSN_PACKAGE arg
	;

path
	: ADM_CMD_DSN_PATH arg
	;
...
version
	: ADM_CMD_DSN_VERSION arg
	;

bindQuery
	: ADM_CMD_DSN_BIND ADM_CMD_DSN_QUERY (bindQueryOptions continuation?)+
	;
...
dsn
	: ADM_CMD_DSN_DSN (dsnOptions continuation?)+
	;
...
free
	: ADM_CMD_DSN_FREE (freeOptions continuation?)+
	;
...
stabilized_dynamic_query
	: ADM_CMD_DSN_STABILIZED continuation* ADM_CMD_DSN_DYNAMIC continuation* ADM_CMD_DSN_QUERY
	;
...
rebindTrigger
	: REBIND ADM_CMD_DSN_TRIGGER continuation? (bindOptions continuation?)+
	;

arg
	: ADM_CMD_DSN_LPAREN (ADM_CMD_DSN_LPAREN | ARG+ | QUOTED_STRING_FRAGMENT+ | SQUOTE2+ | ADM_CMD_DSN_RPAREN)* ADM_CMD_DSN_RPAREN
	;

libargs
	: ADM_CMD_DSN_LPAREN ((ARG+ libmember?) | QUOTED_STRING_FRAGMENT+) ARG* ADM_CMD_DSN_RPAREN
	;

libmember
	: (ADM_CMD_DSN_LPAREN ARG+ ADM_CMD_DSN_RPAREN)
	;
...
/* End of admin_command_dsn parsing section */
...
processingType
    // /* TODO: when processingtype is used, db2command should be written out fully, not abbreviated. 
    // Add check for that */
    : (ADM_CMD_DB2_EMPTYARG | ADM_CMD_DB2_NULL | processingTypes)
    ;

processingTypes
    : ADM_CMD_DB2_SIMPLEQUOTE (BP | ADM_CMD_DB2_DB | TS | IX | THD | UT | GRP | DDF | SYC) ADM_CMD_DB2_SIMPLEQUOTE
    ;

db2Member
    : (ADM_CMD_DB2_EMPTYARG | ADM_CMD_DB2_NULL | DB2MEMBER)
    ;
...
// TODO: Provide for quoted string as param name. Does not work properly yet
returnMsg
	// : ADM_CMD_DB2_RETURNCODE
    : (ADM_CMD_DB2_EMPTYARG | ADM_CMD_DB2_SIMPLEQUOTE? ADM_CMD_DB2_INTEGER ADM_CMD_DB2_SIMPLEQUOTE?)
	;

admCmdDb2ReturnCode
    // : ADM_CMD_DB2_RETURNCODE
	: (ADM_CMD_DB2_SIMPLEQUOTE? ADM_CMD_DB2_INTEGER  ADM_CMD_DB2_SIMPLEQUOTE? | ADM_CMD_DB2_EMPTYARG)
	;

// returnCode
//     :(SIMPLEQUOTE? INTEGER SIMPLEQUOTE? | ADM_CMD_DB2_EMPTYARG)
//     ;
...
// arg
// 	: LPAREN (LPAREN | SPLAT | ARG+ | QUOTED_STRING_FRAGMENT+ | SQUOTE2+ | RPAREN)* RPAREN
// 	;

// continuation
// 	: (DASH | PLUS)
// 	;
/* End ADMIN_COMMAND_DB2 parsing section */
...
utStmts
    : UTIL_SIMPLEQUOTE (listdef+ | template+)  utilities+ UTIL_SIMPLEQUOTE
    ;

utilities
    : (reorg | imageCopy | modifyRecovery | load | exec | runStats | quiesce | rebuildIndex)
    ;

reorg
    : UTIL_REORG UTIL_TABLESPACE (LIST | OBJECTNAME) UTIL_LOG NOSYSREC COPYDDN? UNLDDN? SORTDEVT SORTNUM statistics? SHRLEVEL SORTKEYS?
    ;

imageCopy
    : UTIL_COPY (LIST | OBJECTNAME) COPYDDN UTIL_PARALLEL? SHRLEVEL modifyRecovery?
    ;

modifyRecovery
    : MODIFY RECOVERY (LIST | OBJECTNAME) DSNUM MODRCV_DELETE
    ;

load
    : LOAD_DATA INCURSOR (RESUME | UTIL_REPLACE) COPYDDN? NOCOPYPEND? NOCHECKPEND? UTIL_LOG SORTKEYS ENFORCE statistics? WORKDDN SORTDEVT SORTNUM LOAD_INTO UTIL_TABLE TABLENAME statistics?
    ;
...
runStats
    : RUNSTATS UTIL_TABLESPACE (LIST | OBJECTNAME) INVALIDATECACHE REPORT STATS_UPDATE
    ;

rebuildIndex
    : REBUILD statIndexSpec SHRLEVEL? SORTDEVT SORTNUM  statistics? STATS_UPDATE? UTIL_PARALLEL?
    ;

quiesce
    : QUIESCE (LIST | OBJECTNAME)
    ;

 // TODO: Works for now, but refine, maybe even split up, for reorg, load, rebuild statements later.
statistics
    : STATISTICS statTableSpec? statIndexSpec? corrStatSpec?
    ;

statTableSpec
    : UTIL_TABLE ALL_ITEMS
    ;

statIndexSpec
    : UTIL_INDEX (ALL_ITEMS | LIST | OBJECTNAME) | INDEXSPACE (ALL_ITEMS | LIST | OBJECTNAME)
    ;

corrStatSpec
    : KEYCARD? FREQVAL NUMCOLS STATS_COUNT
    ;

listdef
    : LISTDEF (includeTablespace | includeIndex | includeIndexspace)+
    ;

includeTablespace
    : UTIL_INCLUDE UTIL_TABLESPACE OBJECTNAME
    ;

includeIndexspace
    : UTIL_INCLUDE INDEXSPACE OBJECTNAME
    ;

includeIndex
    : UTIL_INCLUDE UTIL_INDEX OBJECTNAME
    ;

template
    : TEMPLATE UTIL_DSN UTIL_LPAREN DATASETNAME UTIL_RPAREN UNIT? VOLCNT? TMPL_STORCLAS? RETPD TEMPLATE_SPACE? DISP? STACK? listdef?
    ;
...
/* End of ADMIN_COMMAND and DSNUTILU section */ 
